...
const StackCard = ({ project, index, total, scrollYProgress }) => {
    ...
    // This card slides up into view during its window
    const y = useTransform(
        scrollYProgress,
        [cardStart - 0.1, cardStart + 0.2],
        ["80%", "0%"]
    );
    const opacity = useTransform(
        scrollYProgress,
        [cardStart - 0.05, cardStart + 0.15],
        [0, 1]
    );

    // Slight scale-down + push back as the next card covers it
    const scale = useTransform(
        scrollYProgress,
        [cardEnd - 0.05, cardEnd + 0.15],
        [1, 0.96]
    );
    const yPush = useTransform(
        scrollYProgress,
        [cardEnd - 0.05, cardEnd + 0.15],
        ["0%", "-3%"]
    );
            ...
            >
                ...
                <div className="grid grid-cols-2 gap-0 h-full relative z-10">
                    ...
                    <div className="p-8 flex flex-col justify-between" style={{ minHeight: 340 }}>
                        <div>
                            <div className="text-xs text-accent-20 mb-2 uppercase tracking-widest">
                                {project.year}
                            </div>
                        ...
                        <Button
                            size="sm"
                            className="mt-6 rounded-full bg-accent-30 hover:bg-purple-600 text-purple-200 border border-purple-500/30 w-fit"
                        >
                            View project →
                        </Button>
                    </div>
...
export default function PortfolioLayered() {
    ...
    return (
        <section
            ref={ref}
            className="relative bg-black text-white flex justify-center"
        ...
        >
            <div className="w-[70%] relative">
                <div className="sticky top-0 h-screen flex flex-col items-center justify-center overflow-hidden">
                    ...
                    {/* Header — fades out as first card settles */}
                    <div
                       
                        className="relative z-0 text-center mb-16 px-4"
                    >
                        <div className="inline-block px-4 py-1 mb-4 rounded-full bg-accent/20 text-accent text-sm">
                            Portfolio
                        </div>
                    ...
                    {/* Card counter */}
                    <motion.div className="flex gap-2 mt-6 relative z-10">
                        {projects.map((_, i) => (
                            <motion.div
                                key={i}
                                style={{
